...
|-----------|--------------|------------------|
| **"계획을 세워줘"** | `/prd` | "기획서 작성", "PRD 만들어줘", "계획 세워", "Planning", "요구사항 정리", "기획 필요해" |
| **"구조를 검토해줘"** | `/arch-review` | "아키텍처 봐줘", "설계 검토", "구조 어떰?", "Architecture review", "설계 확인" |
| **"성능이 문제야"** | `/bottleneck`, `/profile`, `/bench` | "느려", "최적화", "병목", "성능", "Slow", "Optimize", "Bottleneck", "Performance issue" |
| **"보안을 점검해줘"** | `/audit` | "보안 점검", "취약점", "Security check", "Audit", "Secure?", "보안 검사" |
| **"복잡해서 어려워"** | `/complexity` | "복잡해", "복잡도", "너무 어려워", "Complex", "Too complicated", "복잡도 분석" |
| **"속도를 비교해줘"** | `/bench` | "벤치마크", "성능 비교", "빠른 게 뭐?", "Benchmark", "Compare", "Which is faster?" |
| **"메모리 문제"** | `/mem-check` | "메모리 누수", "Memory leak", "메모리", "RAM", "메모리 문제" |
| **"코드를 검토해줘"** | `/review` | "리뷰해줘", "검토해봐", "코드 봐줘", "Review this", "Check code", "피드백줘" |
| **"API 문서를 추출해줘"** | `/api-docs` | "API 문서", "API docs", "docstring 변환", "자동 문서화" |
| **"변경 내역을 정리해줘"** | `/changelog` | "변경 로그", "CHANGELOG", "커밋 내역 정리", "변경사항 정리" |
| **"버전을 올려줘"** | `/bump` | "버전 업", "태그 생성", "release", "배포 준비" |
| **"코드를 공유할게"** | `/push-safe` 또는 `/update` | "올려", "푸시", "공유", "Push", "동기화", "코드 올리기" |
| **"코드 포맷 점검"** | `/format-check` | "포맷 확인", "lint check", "스타일 검사" |
| **"품질 검사해줘"** | `/lint-smart` | "린트", "코드 품질", "quality check" |
| **"아이디어를 찾아줘"** | `/brainstorm` | "아이디어", "브레인스토밍", "생각해", "아이디어링" |
| **"진행 상황이 궁금해"** | `/stats` 또는 `/trace` | "통계", "현황", "status", "진행 상황", "pipeline trace" |
| **"작업 상태 저장할게"** | `/save-point` | "저장", "세이브", "checkpoint", "상태 저장" |
| **"빨리 핫픽스할게"** | `/quick` | "빨리 수정", "핫픽스", "quick fix" |
| **"PRD 검증해줘"** | `/gate` | "PRD 검증", "gate", "품질 확인" |
| **"전체 파이프라인 실행"** | `/pipeline` | "파이프라인", "전체 실행", "자동화" |
| **"초기화할게"** | `/init` | "설정", "초기화", "setup", "설정 초기화" |
| **"작업 모드 바꿀게"** | `/mode` | "모드 변경", "solo", "team" |
